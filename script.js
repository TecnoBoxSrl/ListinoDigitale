...
// Stato
const state = { items:[], categories:[], selectedCategory:'Tutte', search:'', sort:'alpha',
  onlyAvailable:false, onlyNew:false, priceMax:null, role:'guest' };
...
function setupUI(){
  ...
  // Ricerca/filtri/sort (solo se gli elementi esistono)
...
async function signOut(){
  ...
  state.role='guest'; state.items=[]; applyFilters(); await renderAuthState();
}
...
async function fetchProducts(){
  try{
    ...
    for (const p of (data||[])){
      ...
      if (video){ const { data: vBlob } = await supabase.storage.from('media').download(video); if (vBlob) videoUrl = URL.createObjectURL(vBlob); }
      items.push({ codice:p.codice, descrizione:p.descrizione, categoria:p.categoria, sottocategoria:p.sottocategoria,
        prezzo:p.prezzo, unita:p.unita, disponibile:p.disponibile, novita:p.novita, pack:p.pack, pallet:p.pallet,
        tags:p.tags||[], img:images[0]||'', img2:images[1]||'', img3:images[2]||'', video:videoUrl, updated_at:p.updated_at });
    }
    state.items = items;
    buildCategories();
    applyFilters();
...
function buildCategories(){
  ...
  const box = $('categoryList'); if (!box) return; box.innerHTML='';
  state.categories.forEach(cat=>{
    const b=document.createElement('button'); b.className='tag hover:bg-slate-100'; b.textContent=cat;
